...
national <- pathways %>% 
  ...
  mutate(total_gender=sum(triagecount)) %>% 
  ungroup()
...
ggplot(national, aes(x=date, y=total_nat)) + 
  ...
  scale_x_date(date_breaks = "1 day") + scale_y_continuous(labels = scales::comma, limits = c(0, NA)) +
...
ggplot(national) + 
  ...
  geom_point(aes(x=date, y=total_gender), color='white', size=1) + 
  ...
  labs(caption='Source: NHS Digital')
ggsave(filename=here::here('output', 'pathways_gender.png') ) 
